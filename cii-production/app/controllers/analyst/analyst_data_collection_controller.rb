class Analyst::AnalystDataCollectionController < ApplicationController
    before_action :authenticate_user!
    before_action :set_data_collection_answer, only: [:data_collection_assessment, :view_providedata, :fetch_data_collection_data]
    before_action :set_data_collection_company, only: [:data_collection_assessment, :submit_providedata, :view_providedata, :fetch_companydata, :destroy, :data_automation, :data_collection_company]
    ...
    def index
      ...
      respond_to do |format|
        format.html
        format.js { render partial: 'analyst/analyst_data_collection/data_collection_user_table' }
      end
    end
    ...
    def submit_providedata
      ...
      redirect_to analyst_data_collection_path, notice: "Response has been submitted successfully!"
    end
    ...
    def clear
      ...
      answer.destroy
      redirect_to analyst_data_collection_assessment_path(subscription_type: params[:subscription_type]), notice: "Data has been cleared successfully!"
    end
    ...
    def destroy
      ...
      redirect_to analyst_data_collection_path, notice: 'Company was deleted successfully.'
    end
    ...
    def data_automation
      ...
      redirect_to analyst_data_collection_assessment_path, notice: notice_msg
    end
    ...
    def data_collection_company
      ...
      redirect_to analyst_data_collection_path(page: @page, tab: @tab), notice: msg
    end
    ...
    def save_answer
      ...
      company = find_company(params[:company_id])
      company_hidden_name = params[:company_hidden_name].strip.squeeze(' ')           # hidden field company name
      company_name = company.company_name.strip.squeeze(' ')                            # params company name
      if company_hidden_name != company_name || params[:company_id].to_s != params[:company_hidden_id].to_s      # checking both company names are same or not
    ...
    def set_data_collection_company
      @company = DataCollectionCompanyDetail.find_by(id: params[:company_id])
    end

    def set_data_collection_answer
      @data = DataCollectionAnswer.find_by(company_id: params[:company_id], version: current_year)
      if @data.present?
        @answer = DataCollectionAnswer.find_by(company_id: params[:company_id], version: current_year)
      else
        @answer =  DataCollectionAnswer.where(company_id: params[:company_id]).order(version: :desc).first
      end
    end
end
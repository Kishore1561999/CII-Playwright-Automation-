class Manager::CompanyUsersController < ApplicationController
  before_action :authenticate_user!
  before_action :set_company_user, only: [:edit, :show, :update, :destroy, :update_status]
  skip_before_action :verify_authenticity_token, only: [:update_status, :assign_analyst]
  ...
  def create
    ...
    respond_to do |format|
      if @company_user.save
        format.html { redirect_to manager_company_users_path, notice: 'Company User has been created successfully.' }
      else
  ...
  def update
    respond_to do |format|
      if @company_user.update(company_user_params)
        format.html { redirect_to manager_company_users_path, notice: 'Company User has been updated successfully.' }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to manager_company_users_path, notice: 'Company User has been deleted successfully.' }
    end
  end
  ...
  def update_status
    ...
    Rails.logger.info "***update_params***#{@company_user&.id} -manager-- #{update_params} --- #{params} --- #{questionnaire_version_id}"
    ...
    if @company_user.update(update_params)
      ...
      redirect_to manager_company_users_path, notice: 'Company User request status has been updated successfully.'
    else
      redirect_to manager_company_users_path, error: 'Company User request status update has been failed.'
    end
  end
  ...
  def assign_analyst
    ...
    redirect_to manager_company_users_path, notice: 'Company User has been successfully assigned to Analyst.'
  end

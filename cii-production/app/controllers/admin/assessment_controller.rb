class Admin::AssessmentController < ApplicationController
  ...
  def edit_assessment
    ...
    if ApplicationRecord::REPORT_DOWNLOAD_ENABLE_STATUS.include?(@company_user.user_status) || params[:user_type] != ApplicationRecord::CII_USER || !AssignAnalyst.find_by(company_user_id: params[:company_user_id], analyst_user_id: current_user.id).present?
      redirect_to admin_company_users_path, notice: "Un-processable request!"
    end
  ...
  def submit_assessment
    ...
    redirect_to admin_company_users_path, notice: "Response has been updated successfully!"
  end
  ...
  private
  def save_answer
    Rails.logger.info "Admin Answered_to #{params[:company_user_id]}"
    ...
    Rollbar.error('Admin Assessment Save', e)
  end
end

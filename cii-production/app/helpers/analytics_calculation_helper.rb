...
module AnalyticsCalculationHelper
  ...
  def calculate_average(total_answers_data, question_name, company_data, qid)
    ...
    max = allvalues.max || 0
    min = allvalues.min || 0
    { average: average, maximum: max.round(2), minimum: min.round(2), question_name: question_name, size: total_count, sum: sumvalue, allvalues: allvalues}
  end

  def calculate_average_table_data(total_answers_data, question_name, company_data, qid)
    result = extract_numeric_answers(total_answers_data, question_name).reject(&:nil?)
    return { average: '0', maximum: '0', minimum: '0' } if result.empty?
    if qid == 'PB-05' || qid == 'PB-06' || qid == 'PB-07' || qid == 'PB-09' || qid == 'PB-13'
      name_value = 'Information available'
    else
      name_value = 'information_available'
    end
    ...
    max = allvalues.max || 0
    min = allvalues.min || 0
    { average: average, maximum: max, minimum: min, question_name: question_name, size: total_count, sum: sumvalue, allvalues: allvalues }
  end
  ...
  def calculate_percentage_seventhquestion(total_answers_data, question_data, q_id)
    ...
    if q_id != 'PB-14' ||  q_id != 'PB-28' ||  q_id != 'PB-29' ||  q_id != 'PB-30' ||  q_id != 'PB-31' ||  q_id != 'PB-32' 
  ...
  def calculate_ethics(total_answers_data, question_name, q_id)
    ...
    total_answers_data.each do |answer|
      ...
      #count_option_a += 1 if option_a.present?
      #count_option_b += 1 if option_b.present?
      #count_both_a_and_b += 1 if option_a.present? && option_b.present?
